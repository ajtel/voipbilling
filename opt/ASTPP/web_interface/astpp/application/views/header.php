...
<head>
 ...
 <title>
	...
	if (isset($user_header) && $user_header != '') { ?>
		<? start_block_marker('page-title') ?><? end_block_marker() ?> | AJTEL Comunicaciones&reg; Voip Billing Solution
	<?php
	} else { ?>
		<? start_block_marker('page-title') ?><? end_block_marker() ?> | AJTEL Comunicaciones&reg; Voip Billing Solution
	<?php
...
<nav class="navbar navbar-expand-lg navbar-light bg-light mainmenu px-lg-4">
 <? if ($this->session->userdata('userlevel_logintype') != '0') {
		...
		?> 
                	...
                	<img id="logo" class="img-fluid" alt="user_logo" src="<?php echo base_url(); ?>upload/<?= $user_logo['logo']?>" style="max-height:52px;width:auto;">
		<? }?>
                </a>
